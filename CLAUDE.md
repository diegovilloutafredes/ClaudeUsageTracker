...
- **MenuBarView.swift** — Popover UI showing per-window progress bars, utilization %, reset countdowns, subscription badge, and extra usage. All spacing, padding, and font sizes are multiplied by `s = CGFloat(viewModel.popupScale)`. `private var s` and a `sf(_:_:) -> Font` helper centralize the scaling math. `baseWidth` is `312 * s` (pre-scaled by the ×1.05 rebase applied to all constants).
- **SettingsView.swift** — Account status, update checker (shows green badge + Download link when an update is available, or "Check for Updates" button), subscription badge, menu bar window picker, popup scale slider (75–150%), notification controls (toast/sound/banner, duration, permanent mode), refresh interval slider, pace indicator toggle with rate window picker (5/10/15/30 min), and pace alert controls. All `Toggle` views use `GreenSwitchStyle`. The window title is set by `SettingsWindowPositioner.makeNSView` to `"Settings · v\(version)"`. The Account section uses a footer with the unofficial API disclaimer.
